...
def display_Test_Date_1_panel():
    #header image
    st.image("https://i.imgur.com/n9251uV.jpeg", use_container_width=True)
    st.markdown("---")

    st.title("Test Date 1 Page")
    ...
    def assign_groups(df, num_groups):
        try:
            df = df.sample(frac=1, random_state=42).reset_index(drop=True)  # Randomize data
